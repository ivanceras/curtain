...
fn main(){
	...
	let is_owned = order_line.is_owned(&all_tables);
	...
	println!("order is owned: {}", orders.is_owned(&all_tables));
	...
	for ref table in &all_tables{
		...
		if table.semi_owned(&all_tables){
