...
impl ToJson for TestStruct {
    fn to_json(&self) -> Json {
        ...
        d.insert("updated".to_string(), Json::I64(self.updated.clone().unwrap().timestamp));
        Json::Object(d)
    }
}
...
fn main() {
    ...
    let input_data = TestStruct {
        ...
        created:MyDate{timestamp:UTC::now().timestamp()},
        updated:Some(MyDate{timestamp:UTC::now().timestamp()}),
    };
